...
class BlockController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'Block';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new Block($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'BlockValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        foreach (Block::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(Block::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(Block::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(Block::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(Block::$_meta, $values);
        $this->fv->MakeEnums(Block::$_meta, $this->enums);

        $meta = $this->xmlutils->Meta2XML(Block::$_meta);

        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(Block::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(Block::$_meta, array_merge($_REQUEST, $_FILES), $fields);
