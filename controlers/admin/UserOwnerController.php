...
require_once 'models/UserOwner.php';
require_once 'models/User.php';
    
class UserOwnerController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'UserOwner';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new UserOwner($this->cmf);
        $this->fv     = new \sabay\FormValidator($this->cmf, $this, 'UserOwnerValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        foreach (UserOwner::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(UserOwner::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(UserOwner::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(UserOwner::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(UserOwner::$_meta, $values);
        $this->fv->MakeEnums(UserOwner::$_meta, $this->enums);

        $meta = $this->cmf->Meta2XML(UserOwner::$_meta);

        $values_xml = $this->cmf->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(UserOwner::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(UserOwner::$_meta, array_merge($_REQUEST, $_FILES), $fields);
