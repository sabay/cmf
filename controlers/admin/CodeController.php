...
class CodeController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
    ...
    {
        $this->item   = new Code($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'CodeValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        if (!$count) {
            $count = $this->db->fetchOne('SELECT COUNT(*) FROM bcode WHERE block_id=?', $this->pid);
        }
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'bcode'), Code::$_meta_select)
                           ->where('a.block_id=?', $this->pid)
;

        foreach (Code::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        if (isset($s) && $s) {
            ...
            $select->order(Code::$_meta_order[$s_name][$s_order]);
        }
        ...
        $rows = $stmt->fetchAll();
        $this->fv->PreVisible(Code::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null,$values=array())
    {
        ...
        $this->fv->PiceImages(Code::$_meta, $values);
        $this->fv->MakeEnums(Code::$_meta, $this->enums);
        $meta = $this->xmlutils->Meta2XML(Code::$_meta);
    
        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(Code::$_meta, $values);
   ...
   {
        $fields = array();
        $this->errors = $this->fv->Validate(Code::$_meta,
