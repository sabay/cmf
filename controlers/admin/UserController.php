...
class UserController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'User';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new User($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'UserValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        if (isset($_COOKIE['filter_values'])) {
...
//        $this->cmf->setFilter($select, $values, $this->item->_meta_filter);
        ...
        foreach (User::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(User::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(User::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(User::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(User::$_meta, $values);
        $this->fv->MakeEnums(User::$_meta, $this->enums);

        $meta = $this->xmlutils->Meta2XML(User::$_meta);

        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(User::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(User::$_meta, array_merge($_REQUEST, $_FILES), $fields);
