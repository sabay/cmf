...
require_once 'models/IgnoreDomains.php';
require_once 'models/User.php';
    
class IgnoreDomainsController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'IgnoreDomains';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new IgnoreDomains($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'IgnoreDomainsValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'ignore_domains'), 'count(*)');
        ...
        foreach (IgnoreDomains::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(IgnoreDomains::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(IgnoreDomains::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(IgnoreDomains::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(IgnoreDomains::$_meta, $values);
        $this->fv->MakeEnums(IgnoreDomains::$_meta, $this->enums);

        $meta = $this->xmlutils->Meta2XML(IgnoreDomains::$_meta);

        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(IgnoreDomains::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(IgnoreDomains::$_meta, array_merge($_REQUEST, $_FILES), $fields);
