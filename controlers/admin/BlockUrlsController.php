...
require_once 'models/BlockUrls.php';
        
require_once 'models/User.php';
    

class BlockUrlsController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $item     = null;
    protected $scriptName = 'BlockUrls';
    ...
    {
        $this->item   = new BlockUrls($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'BlockUrlsValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        if (!$count) {
            $count = $this->db->fetchOne('SELECT COUNT(*) FROM blockurls WHERE block_id=?', $this->pid);
        }
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'blockurls'), BlockUrls::$_meta_select)
                           ->where('a.block_id=?', $this->pid)
;

        foreach (BlockUrls::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        if (isset($s) && $s) {
            ...
            $select->order(BlockUrls::$_meta_order[$s_name][$s_order]);
        }
        ...
        $rows = $stmt->fetchAll();
        $this->fv->PreVisible(BlockUrls::$_meta, $rows, $this->enums);
        $rows_xml = $this->xmlutils
                         ->getXML($rows,
                                  array('rootTag'    => 'rows',
                                        'rowTag'     => 'row',
                                        'attributes' => array('_id' => 'id', '_state' => 'status'),
    ...
    public function editAction($id=null,$values=array())
    {
        ...
        $this->fv->PiceImages(BlockUrls::$_meta, $values);
        $this->fv->MakeEnums(BlockUrls::$_meta, $this->enums);
        $meta = $this->xmlutils->Meta2XML(BlockUrls::$_meta);
    
        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(BlockUrls::$_meta, $values);
   ...
   {
        $fields = array();
        $this->errors = $this->fv->Validate(BlockUrls::$_meta,
