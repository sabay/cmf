...
require_once 'models/PartnerSystem.php';
require_once 'models/User.php';
    
class PartnerSystemController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'PartnerSystem';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new PartnerSystem($this->config);
        $this->fv     = new \sabay\FormValidator($this->config, $this, 'PartnerSystemValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'partner_system'), 'count(*)');
        ...
        foreach (PartnerSystem::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(PartnerSystem::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(PartnerSystem::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(PartnerSystem::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(PartnerSystem::$_meta, $values);
        $this->fv->MakeEnums(PartnerSystem::$_meta, $this->enums);

        $meta = $this->xmlutils->Meta2XML(PartnerSystem::$_meta);

        $values_xml = $this->xmlutils->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(PartnerSystem::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(PartnerSystem::$_meta, array_merge($_REQUEST, $_FILES), $fields);
