...
class RestrictionController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'Restriction';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new Restriction($this->cmf);
        $this->fv     = new \sabay\FormValidator($this->cmf, $this, 'RestrictionValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'restriction'), 'count(*)');
        ...
        foreach (Restriction::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(Restriction::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(Restriction::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(Restriction::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(Restriction::$_meta, $values);
        $this->fv->MakeEnums(Restriction::$_meta, $this->enums);

        $meta = $this->cmf->Meta2XML(Restriction::$_meta);

        $values_xml = $this->cmf->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(Restriction::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(Restriction::$_meta, array_merge($_REQUEST, $_FILES), $fields);
