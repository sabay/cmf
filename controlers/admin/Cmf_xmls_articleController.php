...
require_once 'models/Cmf_xmls_article.php';
require_once 'models/User.php';
    
class Cmf_xmls_articleController extends sabay\Abstract_Admin_Controller
{
    /**
     * @var User
     */
     ...
     */
    protected $fv      = null;
    protected $scriptName = 'Cmf_xmls_article';
    
    protected $defaultValues = array();

    public function init()
    {
        $this->item   = new Cmf_xmls_article($this->cmf);
        $this->fv     = new \sabay\FormValidator($this->cmf, $this, 'Cmf_xmls_articleValidator');
        $this->errors = array();
    ...
    public function indexAction()
    {
        ...
        $select = $this->db->select()
                           ->from(array('a'=>'cmf_xmls_article'), 'count(*)');
        ...
        foreach (Cmf_xmls_article::$_meta_order as $name => $order) {
            if (
                isset($order[2])
                && is_array($order[2])
            ) {
        ...
        $select->columns(Cmf_xmls_article::$_meta_select);
        ...
        if (isset($s) && $s) {
            ...
            $select->order(Cmf_xmls_article::$_meta_order[$s_name][$s_order]);
        }
        ...
        $this->fv->PreVisible(Cmf_xmls_article::$_meta, $rows, $this->enums);
    ...
    public function editAction($id=null, $values=array())
    {
        ...
        $this->fv->PiceImages(Cmf_xmls_article::$_meta, $values);
        $this->fv->MakeEnums(Cmf_xmls_article::$_meta, $this->enums);

        $meta = $this->cmf->Meta2XML(Cmf_xmls_article::$_meta);

        $values_xml = $this->cmf->Form2XML($values);
        $values_xml .= $this->fv->MultiOptions(Cmf_xmls_article::$_meta, $values);
    ...
    {
        $fields = array();
        $this->errors = $this->fv->Validate(Cmf_xmls_article::$_meta, array_merge($_REQUEST, $_FILES), $fields);
